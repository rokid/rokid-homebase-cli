...
class Device {
  ...
  static updateStateById (endpointId, sessionName, oldStates, newStates) {
    if (oldStates === null) {
      const devicesOfSession = Device.getBySessionName(sessionName)
      const targetDevice = devicesOfSession.find(it => it.endpointId === endpointId)
      oldStates = targetDevice.states
    }
    db.get('devices')
      .find({
        endpointId,
